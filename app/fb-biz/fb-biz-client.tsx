...
import {
  useFbBizBoard,
  ALL_FB_BIZ_POST_TYPES,
  ALL_FB_BIZ_FORMATS,
  type FbBizBoardCard,
  type FbBizStatus,
  type FbBizPostType,
  type FbBizFormat,
} from "@/lib/use-fb-biz-board";

const COLUMNS: { key: FbBizStatus; label: string }[] = [
...
function BoardCardItem({
  ...
  card: FbBizBoardCard;
  index: number;
  onUpdate: (id: string, updates: Partial<FbBizBoardCard>) => void;
  ...
  const [form, setForm] = useState<FbBizBoardCard>({ ...card });
                ...
                <div className="grid grid-cols-3 gap-3">
                  <Select
                    ...
                    onValueChange={(v) =>
                      setForm({
                        ...form,
                        format: v === "none" ? undefined : (v as FbBizFormat),
                      })
                    }
                  >
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setForm({
                        ...form,
                        postType:
                          v === "none" ? undefined : (v as FbBizPostType),
                      })
                    }
                  >
                    <SelectTrigger className="bg-background border-border text-sm h-9">
...
function AddCardDialog({
  ...
  onAdd: (card: Omit<FbBizBoardCard, "id" | "createdAt">) => void;
}) {
  const [open, setOpen] = useState(false);
  const [text, setText] = useState("");
  const [postType, setPostType] = useState<FbBizPostType | "none">("none");
  const [format, setFormat] = useState<FbBizFormat | "none">("none");
  const [status, setStatus] = useState<FbBizStatus>("ideas");
      ...
      <DialogContent className="max-w-md bg-card border-border">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Select
              value={status}
              onValueChange={(v) => setStatus(v as FbBizStatus)}
            >
              <SelectTrigger className="bg-background border-border text-sm h-9">
                <SelectValue />
              </SelectTrigger>
            ...
            <Select
              value={format}
              onValueChange={(v) => setFormat(v as FbBizFormat | "none")}
            >
              <SelectTrigger className="bg-background border-border text-sm h-9">
                <SelectValue placeholder="Format" />
              </SelectTrigger>
            ...
            <Select
              value={postType}
              onValueChange={(v) => setPostType(v as FbBizPostType | "none")}
            >
              <SelectTrigger className="bg-background border-border text-sm h-9">
...
export function FbBizClient() {
  ...
  const [mobileCol, setMobileCol] = useState<FbBizStatus>("ideas");
  ...
  function filterCards(col: FbBizStatus) {
  ...
  function onDragEnd(result: DropResult) {
    ...
    const srcCol = result.source.droppableId as FbBizStatus;
    const dstCol = result.destination.droppableId as FbBizStatus;
    const srcIdx = result.source.index;
    const dstIdx = result.destination.index;

    if (srcCol === dstCol && srcIdx === dstIdx) return;

    const colCards = (col: FbBizStatus) =>
    ...
    if (srcCol === dstCol) {
      ...
      const colOrder: FbBizStatus[] = [
        "ideas",
        "drafts",
        "scheduled",
        "published",
      ];
      const rebuild: FbBizBoardCard[] = [];
