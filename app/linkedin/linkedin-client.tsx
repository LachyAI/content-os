...
import {
  useLinkedInBoard,
  ALL_LINKEDIN_POST_TYPES,
  type LinkedInBoardCard,
  type LinkedInStatus,
  type LinkedInPostType,
} from "@/lib/use-linkedin-board";

const COLUMNS: { key: LinkedInStatus; label: string }[] = [
...
function BoardCardItem({
  ...
  card: LinkedInBoardCard;
  index: number;
  onUpdate: (id: string, updates: Partial<LinkedInBoardCard>) => void;
  ...
  const [form, setForm] = useState<LinkedInBoardCard>({ ...card });
              ...
              {card.status === "published" &&
                card.reactions !== undefined && (
                  <div className="ml-5 mt-2 flex items-center gap-2 text-[10px] text-muted-foreground flex-wrap">
                    ...
                    {card.reposts !== undefined && (
                      <span>{card.reposts} reposts</span>
                    )}
                    {card.impressions !== undefined && (
                      <span>{card.impressions?.toLocaleString()} impressions</span>
                    )}
                    {card.url && (
                      <a
                        href={card.url}
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setForm({
                        ...form,
                        postType:
                          v === "none" ? undefined : (v as LinkedInPostType),
                      })
                    }
                  >
...
function AddCardDialog({
  ...
  onAdd: (card: Omit<LinkedInBoardCard, "id" | "createdAt">) => void;
}) {
  const [open, setOpen] = useState(false);
  const [text, setText] = useState("");
  const [postType, setPostType] = useState<LinkedInPostType | "none">("none");
  const [status, setStatus] = useState<LinkedInStatus>("ideas");
      ...
      <DialogContent className="max-w-md bg-card border-border">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            <Select
              value={status}
              onValueChange={(v) => setStatus(v as LinkedInStatus)}
            >
              <SelectTrigger className="bg-background border-border text-sm h-9">
                <SelectValue />
              </SelectTrigger>
            ...
            <Select
              value={postType}
              onValueChange={(v) => setPostType(v as LinkedInPostType | "none")}
            >
              <SelectTrigger className="bg-background border-border text-sm h-9">
...
export function LinkedInClient() {
  ...
  function onDragEnd(result: DropResult) {
    ...
    const srcCol = result.source.droppableId as LinkedInStatus;
    const dstCol = result.destination.droppableId as LinkedInStatus;
    const srcIdx = result.source.index;
    const dstIdx = result.destination.index;

    if (srcCol === dstCol && srcIdx === dstIdx) return;

    const colCards = (col: LinkedInStatus) =>
    ...
    if (srcCol === dstCol) {
      ...
      const colOrder: LinkedInStatus[] = [
        "ideas",
        "drafts",
        "scheduled",
        "published",
      ];
      const rebuild: LinkedInBoardCard[] = [];
  ...
  const counts = COLUMNS.reduce(
    ...
    {} as Record<LinkedInStatus, number>
  );
