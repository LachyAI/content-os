...
import {
  useTiktokBoard,
  ALL_TIKTOK_POST_TYPES,
  ALL_TIKTOK_FORMATS,
  type TiktokBoardCard,
  type TiktokStatus,
  type TiktokPostType,
  type TiktokFormat,
} from "@/lib/use-tiktok-board";

const COLUMNS: { key: TiktokStatus; label: string }[] = [
...
function BoardCardItem({
  ...
  card: TiktokBoardCard;
  index: number;
  onUpdate: (id: string, updates: Partial<TiktokBoardCard>) => void;
  ...
  const [form, setForm] = useState<TiktokBoardCard>({ ...card });
                ...
                card.views !== undefined && (
                  <div className="ml-5 mt-2 flex items-center gap-2 text-[10px] text-muted-foreground flex-wrap">
                    ...
                    {card.likes !== undefined && (
                      <span>{card.likes?.toLocaleString()} likes</span>
                    )}
          ...
          <Dialog open={editOpen} onOpenChange={setEditOpen}>
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <Select
                    ...
                    onValueChange={(v) =>
                      setForm({
                        ...form,
                        format: v === "none" ? undefined : (v as TiktokFormat),
                      })
                    }
                  >
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setForm({
                        ...form,
                        postType:
                          v === "none" ? undefined : (v as TiktokPostType),
                      })
                    }
                  >
                    <SelectTrigger className="bg-background border-border text-sm h-9">
                ...
                {(form.format === "video" || form.format === "story") && (
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
                      Video script
                    </label>
                    <Textarea
                      placeholder="Hook, beats, CTA — paste your video script here"
                      value={form.script ?? ""}
                      onChange={(e) =>
...
function AddCardDialog({
  ...
  onAdd: (card: Omit<TiktokBoardCard, "id" | "createdAt">) => void;
}) {
  const [open, setOpen] = useState(false);
  const [text, setText] = useState("");
  const [postType, setPostType] = useState<TiktokPostType | "none">("none");
  const [format, setFormat] = useState<TiktokFormat | "none">("none");
  const [status, setStatus] = useState<TiktokStatus>("ideas");
  const [scheduledDate, setScheduledDate] = useState("");
  const [script, setScript] = useState("");

  const showScript = format === "video" || format === "story";
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md bg-card border-border">
        ...
        <div className="space-y-3">
          <Textarea
            placeholder="Write a caption/hook or paste an idea..."
            value={text}
          ...
          <div className="grid grid-cols-2 gap-2">
            <Select
              value={status}
              onValueChange={(v) => setStatus(v as TiktokStatus)}
            >
              <SelectTrigger className="bg-background border-border text-sm h-9">
                <SelectValue />
              </SelectTrigger>
            ...
            <Select
              value={format}
              onValueChange={(v) => setFormat(v as TiktokFormat | "none")}
            >
              <SelectTrigger className="bg-background border-border text-sm h-9">
                <SelectValue placeholder="Format" />
              </SelectTrigger>
            ...
            <Select
              value={postType}
              onValueChange={(v) => setPostType(v as TiktokPostType | "none")}
            >
              <SelectTrigger className="bg-background border-border text-sm h-9">
...
export function TiktokClient() {
  ...
  const [mobileCol, setMobileCol] = useState<TiktokStatus>("ideas");
  ...
  function filterCards(col: TiktokStatus) {
  ...
  function onDragEnd(result: DropResult) {
    ...
    const srcCol = result.source.droppableId as TiktokStatus;
    const dstCol = result.destination.droppableId as TiktokStatus;
    const srcIdx = result.source.index;
    const dstIdx = result.destination.index;

    if (srcCol === dstCol && srcIdx === dstIdx) return;

    const colCards = (col: TiktokStatus) =>
    ...
    if (srcCol === dstCol) {
      ...
      const colOrder: TiktokStatus[] = [
        "ideas",
        "drafts",
        "scheduled",
        "published",
      ];
      const rebuild: TiktokBoardCard[] = [];
